...
const paymentStatusService = require("../../../services/v1/admin/paymentStatusService");
const CustomError = require("../../../utils/customErrorHandler");




exports.savePaymentStatus = async (req, res, next) => {
    ...
    try {
        ...
        if (updateId) {
            ...
            const updateResult = await paymentStatusService.updateStatus(data,updateId, transaction);
            if (updateResult) { 
            ...
            const createdResult = await paymentStatusService.createStatus(data, transaction);
            if (createdResult) { 
...
exports.getStatusList = async (req, res, next) => {
    try {
        ...
        const sortField = req.query.sortField || 'payment_status_id'; // Default to 'payment_status_id'
       ...
       const result = await paymentStatusService.getStatusList(queryOptions)
        if (res.pagination) {
...
exports.deleteStatus = async (req, res, next) => {
    try {
        ...
        const statusRes = await paymentStatusService.updateStatus(parseInt(payment_status_id));
        if (!statusRes) {
            res.status(200).send({ status: 0, message: "Status not found" });
        } else {
            const result = await paymentStatusService.DeleteStatus(payment_status_id, ownerId);
            if (result) {
...
exports.updateActiveStatus = async (req, res, next) => {
    ...
    try {
        ...
        if (updateId) {
            ...
            const result = await paymentStatusService.updateStatus(data,updateId, transaction);
            if (result) {
